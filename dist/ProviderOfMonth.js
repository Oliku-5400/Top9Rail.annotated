...
(function () {
  ...
  function fetchProviderOfMonth() {
    ...
    return fetch(url).then(function (res) {
      ...
      if (!payload || _typeof(payload) !== 'object' || !Array.isArray(payload.games)) {
  ...
  function resolveContent(payload, lang) {
    ...
    var games = payload.games.filter(function (g) {
      return g && g.image;
    ...
    return {
      ...
      backgroundMobile: payload.backgroundMobile || '',
      // optional, portrait; mobile falls back to `background`
      ...
      logoBrightness: function (b) {
        b = parseFloat(b);
        return isFinite(b) && b > 0 ? Math.min(3, Math.max(0.5, b)) : 1;
      }(payload.logoBrightness),
  ...
  function useProviderOfMonth(logTag) {
    ...
    React.useEffect(function () {
      var cancelled = false;
      fetchProviderOfMonth().then(function (payload) {
        if (cancelled) return;
  ...
  '.potm,.potm-skeleton{margin:0 2.5px;}', '.potm{position:relative;display:flex;align-items:stretch;height:clamp(360px,31vw,460px);border-radius:16px;overflow:hidden;background:' + INK_JP + ';color:#fff;font-family:Roboto,Arial,sans-serif;isolation:isolate;}', '.potm-bg{position:absolute;inset:0;background-size:cover;background-position:center;z-index:-2;}', '.potm-bg--fallback{background:radial-gradient(120% 140% at 80% 50%,#3a3b37 0%,' + INK_JP + ' 60%);}', '.potm-overlay{position:absolute;inset:0;z-index:-1;background:linear-gradient(90deg,rgba(29,30,27,.92) 0%,rgba(29,30,27,.78) 34%,rgba(29,30,27,.30) 62%,rgba(29,30,27,.45) 100%);}', '.potm-content{flex:0 0 44%;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:40px 0 40px clamp(28px,3.6vw,52px);min-width:0;}',
  ...
  '.potm-logo{display:inline-flex;align-items:center;justify-content:center;height:clamp(68px,6.4vw,92px);padding:0 clamp(22px,2.2vw,32px);max-width:100%;box-sizing:border-box;border-radius:14px;background:rgba(29,30,27,.82);border:1px solid rgba(255,255,255,.14);box-shadow:0 8px 24px rgba(0,0,0,.25);}', '.potm-logo img{height:clamp(30px,2.9vw,42px);width:auto;max-width:clamp(220px,24vw,340px);object-fit:contain;display:block;}', '.potm-logo span{font-weight:700;letter-spacing:.08em;text-transform:uppercase;font-size:18px;}', '.potm-tagline{margin:22px 0 0;font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:rgba(255,255,255,.72);}', '.potm-title{margin:10px 0 0;font-family:Roboto,Arial,sans-serif;font-weight:900;font-style:italic;text-transform:uppercase;letter-spacing:-.01em;line-height:.95;font-size:clamp(34px,3.7vw,58px);color:#fff;text-shadow:0 4px 18px rgba(0,0,0,.35);max-width:12ch;}', '.potm-cta{margin-top:26px;display:inline-flex;align-items:center;gap:8px;padding:12px 22px;border-radius:6px;background:' + RED_JP + ';color:#fff !important;font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;text-decoration:none !important;box-shadow:0 6px 18px rgba(193,2,48,.35);transition:background 160ms ease,transform 160ms ease;}', '.potm-cta:hover{background:#A00228;transform:translateY(-1px);}', '.potm-cta:focus-visible,.potm-card:focus-visible,.potm-dot:focus-visible{outline:2px solid #fff;outline-offset:3px;}', '.potm-stage{flex:1 1 auto;display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:0;outline:none;}', '.potm-track{--potm-card:clamp(170px,15.5vw,240px);position:relative;width:100%;height:var(--potm-card);}', '.potm-card{position:absolute;top:0;left:50%;width:var(--potm-card);height:var(--potm-card);margin-left:calc(var(--potm-card) / -2);border-radius:12px;overflow:hidden;background:' + INK_JP + ';box-shadow:0 14px 34px rgba(0,0,0,.45),0 2px 6px rgba(0,0,0,.25);transition:transform 480ms cubic-bezier(.2,0,0,1),opacity 480ms cubic-bezier(.2,0,0,1),filter 480ms ease;text-decoration:none;color:#fff;-webkit-tap-highlight-color:transparent;}', '.potm-card img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block;}', '.potm-card--side{filter:brightness(.62);cursor:pointer;}', '.potm-card--side:hover{filter:brightness(.8);}', '.potm-card--hidden{pointer-events:none;}', '.potm-scrim{position:absolute;left:0;right:0;bottom:0;padding:26px 12px 10px;background:linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,.6) 45%,rgba(0,0,0,.86) 100%);font-size:11px;line-height:1.35;font-variant-numeric:tabular-nums;letter-spacing:.01em;opacity:0;transform:translateY(8px);transition:opacity 300ms ease 120ms,transform 300ms ease 120ms;}', '.potm-card--active .potm-scrim{opacity:1;transform:none;}', '.potm-scrim-title{font-weight:700;font-size:12px;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}', '.potm-row{display:flex;justify-content:space-between;gap:8px;white-space:nowrap;}', '.potm-row b{font-weight:700;letter-spacing:.06em;}', '.potm-dots{display:flex;gap:8px;margin-top:22px;}', '.potm-dot{width:8px;height:8px;padding:0;border:0;border-radius:4px;background:rgba(255,255,255,.45);cursor:pointer;transition:width 240ms ease,background 240ms ease;}', '.potm-dot:hover{background:rgba(255,255,255,.75);}', '.potm-dot--active{width:24px;background:' + RED_JP + ';}', '.potm-skeleton{height:clamp(360px,31vw,460px);border-radius:16px;background:linear-gradient(90deg,#2a2b28 0%,#34352f 50%,#2a2b28 100%);background-size:200% 100%;animation:potm-sk 1.4s linear infinite;}', '@keyframes potm-sk{from{background-position:200% 0}to{background-position:-200% 0}}', '@media (prefers-reduced-motion:reduce){.potm-card,.potm-scrim,.potm-dot{transition:none}.potm-skeleton{animation:none}}'].join('\n'));
  ...
  function cardStyle(offset) {
    ...
    if (abs === 1) return {
      transform: 'translateX(' + sign * 62 + '%) scale(.82)',
      opacity: 1,
      zIndex: 2
    };
    return {
      transform: 'translateX(' + sign * 110 + '%) scale(.64)',
      opacity: 0,
      zIndex: 1
    };
  }
  ...
  function ProviderOfMonth() {
    ...
    return /*#__PURE__*/React.createElement("section", {
      ...
      onBlur: function onBlur(e) {
        if (!e.currentTarget.contains(e.relatedTarget)) c.setPaused(false);
      }
    }, /*#__PURE__*/React.createElement("div", {
      ...
      className: "potm-overlay"
    }), /*#__PURE__*/React.createElement("div", {
      className: "potm-content"
    }, /*#__PURE__*/React.createElement("div", {
      className: "potm-logo"
    }, content.providerLogo ? /*#__PURE__*/React.createElement("img", {
      src: content.providerLogo,
      alt: content.providerName || 'Provider',
      style: content.logoBrightness !== 1 ? {
        filter: 'brightness(' + content.logoBrightness + ')'
      } : undefined
    }) : /*#__PURE__*/React.createElement("span", null, content.providerName)), content.tagline && /*#__PURE__*/React.createElement("p", {
      className: "potm-tagline"
    }, content.tagline), content.title && /*#__PURE__*/React.createElement("h2", {
      className: "potm-title"
    }, content.title), content.ctaLabel && content.ctaHref && /*#__PURE__*/React.createElement("a", {
      className: "potm-cta",
      href: content.ctaHref,
      onClick: function onClick() {
        trackGoal('potm-click-cta');
      }
    }, content.ctaLabel)), /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("a", {
        ...
        className: "potm-scrim"
      }, g.title && /*#__PURE__*/React.createElement("div", {
        className: "potm-scrim-title"
      }, g.title), g.min && /*#__PURE__*/React.createElement("div", {
        className: "potm-row"
      }, /*#__PURE__*/React.createElement("b", null, "MIN"), /*#__PURE__*/React.createElement("span", null, g.min)), g.max && /*#__PURE__*/React.createElement("div", {
        className: "potm-row"
      }, /*#__PURE__*/React.createElement("b", null, "MAX"), /*#__PURE__*/React.createElement("span", null, g.max))));
